...
from pytracer.hitrecord import HitRecord, Vector2D
from pytracer.shapes.sphere import Sphere
from pytracer.shapes.plane import Plane

from pytracer.math.utils import are_close
from pytracer.world import World
from pytracer.pcg import PCG
from pytracer.materials.brdfs import DiffuseBRDF, SpecularBRDF
from pytracer.materials.pigments import UniformPigment, CheckeredPigment, ImagePigment
...
class TestGeometry(unittest.TestCase):
    ...
    def test_vector_operations(self):
        a = Vector3D(1.0, 2.0, 3.0)
        b = Vector3D(4.0, 6.0, 8.0)
        assert (-a).is_close(Vector3D(-1.0, -2.0, -3.0))
        assert (a + b).is_close(Vector3D(5.0, 8.0, 11.0))
        assert (b - a).is_close(Vector3D(3.0, 4.0, 5.0))
        assert (a * 2).is_close(Vector3D(2.0, 4.0, 6.0))
        assert pytest.approx(40.0) == a.dot(b)
        assert a.cross(b).is_close(Vector3D(-2.0, 4.0, -2.0))
        assert b.cross(a).is_close(Vector3D(2.0, -4.0, 2.0))
...
class TestTransformations(unittest.TestCase):
    ...
    def test_vec_point_multiplication(self):
        ...
        expected_v = Vector3D(14.0, 38.0, 51.0)
        assert expected_v.is_close(m * Vector3D(1.0, 2.0, 3.0))
    ...
    def test_translations(self):
        tr1 = Transformation.from_translation(Vector3D(1.0, 2.0, 3.0))
        assert tr1.is_consistent()

        tr2 = Transformation.from_translation(Vector3D(4.0, 6.0, 8.0))
        assert tr1.is_consistent()

        prod = tr1 * tr2
        assert prod.is_consistent()

        expected = Transformation.from_translation(Vector3D(5.0, 8.0, 11.0))
        assert prod.is_close(expected)

    def test_rotations(self):
        assert Transformation.from_rotation_x(0.1).is_consistent()
        assert Transformation.from_rotation_y(0.1).is_consistent()
        assert Transformation.from_rotation_z(0.1).is_consistent()

        assert (Transformation.from_rotation_x(angle_deg=90) * VEC_Y).is_close(VEC_Z)
        assert (Transformation.from_rotation_y(angle_deg=90) * VEC_Z).is_close(VEC_X)
        assert (Transformation.from_rotation_z(angle_deg=90) * VEC_X).is_close(VEC_Y)

    def test_scalings(self):
        tr1 = Transformation.from_scaling(Vector3D(2.0, 5.0, 10.0))
        assert tr1.is_consistent()

        tr2 = Transformation.from_scaling(Vector3D(3.0, 2.0, 4.0))
        assert tr2.is_consistent()

        expected = Transformation.from_scaling(Vector3D(6.0, 10.0, 40.0))
        assert expected.is_close(tr1 * tr2)


class TestRays(unittest.TestCase):
    def test_is_close(self):
        ray1 = Ray(origin=Point(1.0, 2.0, 3.0), dir=Vector3D(5.0, 4.0, -1.0))
        ray2 = Ray(origin=Point(1.0, 2.0, 3.0), dir=Vector3D(5.0, 4.0, -1.0))
        ray3 = Ray(origin=Point(5.0, 1.0, 4.0), dir=Vector3D(3.0, 9.0, 4.0))

        assert ray1.is_close(ray2)
        assert not ray1.is_close(ray3)

    def test_at(self):
        ray = Ray(origin=Point(1.0, 2.0, 4.0), dir=Vector3D(4.0, 2.0, 1.0))
    ...
    def test_transform(self):
        ray = Ray(origin=Point(1.0, 2.0, 3.0), dir=Vector3D(6.0, 5.0, 4.0))
        transformation = Transformation.from_translation(
            Vector3D(10.0, 11.0, 12.0)
        ) * Transformation.from_rotation_x(90.0)
        transformed = ray.transform(transformation)
        assert transformed.origin.is_close(Point(11.0, 8.0, 14.0))
        assert transformed.dir.is_close(Vector3D(6.0, -4.0, 5.0))
...
class TestCameras(unittest.TestCase):
    ...
    def test_orthogonal_camera_transform(self):
        cam = OrthogonalCamera(
            transformation=Transformation.from_translation(-VEC_Y * 2.0)
            * Transformation.from_rotation_z(angle_deg=90)
        )
    ...
    def test_perspective_camera_transform(self):
        cam = PerspectiveCamera(
            transformation=Transformation.from_translation(-VEC_Y * 2.0)
            * Transformation.from_rotation_z(pi / 2.0)
        )
...
class TestSphere(unittest.TestCase):
    def testHit(self):
        ...
        assert HitRecord(
            ...
            surface_point=Vector2D(0.0, 0.0),
            t=1.0,
            ray=ray1,
        ...
        assert HitRecord(
            ...
            surface_point=Vector2D(0.0, 0.5),
            t=2.0,
            ray=ray2,
            material=sphere.material,
    ...
    def testTransformation(self):
        sphere = Sphere(
            transformation=Transformation.from_translation(Vector3D(10.0, 0.0, 0.0))
        )
    ...
    def testNormals(self):
        sphere = Sphere(
            transformation=Transformation.from_scaling(Vector3D(2.0, 1.0, 1.0))
        )

        ray = Ray(origin=Point(1.0, 1.0, 0.0), dir=Vector3D(-1.0, -1.0))
        intersection = sphere.ray_intersection(ray)
    ...
    def testNormalDirection(self):
        ...
        # reference frame
        sphere = Sphere(
            transformation=Transformation.from_scaling(Vector3D(-1.0, -1.0, -1.0))
        )
    ...
    def testUVCoordinates(self):
        ...
        assert sphere.ray_intersection(ray1).surface_point.is_close(Vector2D(0.0, 0.5))

        ray2 = Ray(origin=Point(0.0, 2.0, 0.0), dir=-VEC_Y)
        assert sphere.ray_intersection(ray2).surface_point.is_close(Vector2D(0.25, 0.5))

        ray3 = Ray(origin=Point(-2.0, 0.0, 0.0), dir=VEC_X)
        assert sphere.ray_intersection(ray3).surface_point.is_close(Vector2D(0.5, 0.5))

        ray4 = Ray(origin=Point(0.0, -2.0, 0.0), dir=VEC_Y)
        assert sphere.ray_intersection(ray4).surface_point.is_close(Vector2D(0.75, 0.5))
...
class TestPlane(unittest.TestCase):
    ...
    def testTransformation(self):
        plane = Plane(transformation=Transformation.from_rotation_y(angle_deg=90.0))
    ...
    def testUVCoordinates(self):
        ...
        ray1 = Ray(origin=Point(0, 0, 1), dir=-VEC_Z)
        intersection1 = plane.ray_intersection(ray1)
        assert intersection1.surface_point.is_close(Vector2D(0.0, 0.0))

        ray2 = Ray(origin=Point(0.25, 0.75, 1), dir=-VEC_Z)
        intersection2 = plane.ray_intersection(ray2)
        assert intersection2.surface_point.is_close(Vector2D(0.25, 0.75))

        ray3 = Ray(origin=Point(4.25, 7.75, 1), dir=-VEC_Z)
        intersection3 = plane.ray_intersection(ray3)
        assert intersection3.surface_point.is_close(Vector2D(0.25, 0.75))
...
class TestWorld(unittest.TestCase):
    def testRayIntersections(self):
        ...
        sphere1 = Sphere(transformation=Transformation.from_translation(VEC_X * 2))
        sphere2 = Sphere(transformation=Transformation.from_translation(VEC_X * 8))
...
class TestPigments(unittest.TestCase):
    def testUniformPigment(self):
        ...
        assert pigment.get_color(Vector2D(0.0, 0.0)).is_close(color)
        assert pigment.get_color(Vector2D(1.0, 0.0)).is_close(color)
        assert pigment.get_color(Vector2D(0.0, 1.0)).is_close(color)
        assert pigment.get_color(Vector2D(1.0, 1.0)).is_close(color)
    ...
    def testImagePigment(self):
        ...
        assert pigment.get_color(Vector2D(0.0, 0.0)).is_close(Color(1.0, 2.0, 3.0))
        assert pigment.get_color(Vector2D(1.0, 0.0)).is_close(Color(2.0, 3.0, 1.0))
        assert pigment.get_color(Vector2D(0.0, 1.0)).is_close(Color(2.0, 1.0, 3.0))
        assert pigment.get_color(Vector2D(1.0, 1.0)).is_close(Color(3.0, 2.0, 1.0))
    ...
    def testCheckeredPigment(self):
        ...
        #   (0, 1) +------+------+ (1, 1)
        #              (0.5, 1)
        assert pigment.get_color(Vector2D(0.25, 0.25)).is_close(color1)
        assert pigment.get_color(Vector2D(0.75, 0.25)).is_close(color2)
        assert pigment.get_color(Vector2D(0.25, 0.75)).is_close(color2)
        assert pigment.get_color(Vector2D(0.75, 0.75)).is_close(color1)


class TestRenderers(unittest.TestCase):
    def testOnOffRenderer(self):
        sphere = Sphere(
            transformation=Transformation.from_translation(Vector3D(2, 0, 0))
            * Transformation.from_scaling(Vector3D(0.2, 0.2, 0.2)),
...
class TestOnbCreation(unittest.TestCase):
    def testOnbFromNormal(self):
        ...
        for i in range(100):
            normal = Vector3D(
                pcg.random_float(), pcg.random_float(), pcg.random_float()
            )
            normal.normalize()
...
class TestPathTracer(unittest.TestCase):
    def testFurnace(self):
        ...
        for i in range(5):
            ...
            ray = Ray(origin=Point(0, 0, 0), dir=Vector3D(1, 0, 0))
...
"""
    ...
    def test_parser(self):
        ...
        assert scene.world.shapes[0].transformation.is_close(
            Transformation.from_translation(Vector3D(0, 0, 100))
            * Transformation.from_rotation_y(150.0)
        )
        ...
        assert scene.world.shapes[2].transformation.is_close(
            Transformation.from_translation(Vector3D(0, 0, 1))
        )
        ...
        assert scene.camera.transformation.is_close(
            Transformation.from_rotation_z(30)
            * Transformation.from_translation(Vector3D(-4, 0, 1))
        )
