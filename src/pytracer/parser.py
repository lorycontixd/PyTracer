...
from .math.geometry import Vector3D, Point
from .hdrimages import read_pfm_image
from .lights import PointLight
from .materials.material import Material
from .materials.pigments import Pigment, UniformPigment, CheckeredPigment, ImagePigment
from .materials.brdfs import BRDF, DiffuseBRDF, SpecularBRDF
from .shapes.plane import Plane
from .shapes.sphere import Sphere
from .math.transformations import Transformation
...
def parse_vector(input_file: InputStream, scene: Scene) -> Vector3D:
...
def parse_transformation(input_file, scene: Scene):
    ...
    while True:
        ...
        if transformation_kw == KeywordEnum.IDENTITY:
            ...
            expect_symbol(input_file, "(")
            result *= Transformation.from_translation(parse_vector(input_file, scene))
            expect_symbol(input_file, ")")
        elif transformation_kw == KeywordEnum.ROTATION_X:
            expect_symbol(input_file, "(")
            result *= Transformation.from_rotation_x(expect_number(input_file, scene))
            expect_symbol(input_file, ")")
        elif transformation_kw == KeywordEnum.ROTATION_Y:
            expect_symbol(input_file, "(")
            result *= Transformation.from_rotation_y(expect_number(input_file, scene))
            expect_symbol(input_file, ")")
        elif transformation_kw == KeywordEnum.ROTATION_Z:
            expect_symbol(input_file, "(")
            result *= Transformation.from_rotation_z(expect_number(input_file, scene))
            expect_symbol(input_file, ")")
        elif transformation_kw == KeywordEnum.SCALING:
            expect_symbol(input_file, "(")
            result *= Transformation.from_scaling(parse_vector(input_file, scene))
            expect_symbol(input_file, ")")
